...
all_years = ['Test']
songs_collection = [path + 'Songs_Test/']
...
def init_dataset (songs_collection):
    ...
    with open("test_indexing.json", "w", encoding="utf-8") as json_file:
...
if __name__ == '__main__':
    ...
    # torch.save(LMD_Dict, 'Test_LMD_Dict_%s.pth'%datetime.datetime.now().strftime("%Y%m%d%H%M%S"))
    
    LMD_Dict = torch.load('Test_LMD_Dict_20230613211934.pth')
    indexing = json.load(open("test_indexing.json", 'r'))
